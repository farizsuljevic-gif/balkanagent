...
assert.doesNotMatch(indexHtml, /<video\b|href="#presentation"|id="presentation"/i);
assert.doesNotMatch(indexHtml, /\.mp4/i);
...
assert.doesNotMatch(indexHtml, /<h3>Enterprise<\/h3>|data-plan="Enterprise"|>Custom<\/div>/);
assert.match(indexHtml, /linear-gradient\(135deg,#a6782e,#f1c768\)/);
assert.match(indexHtml, /Suljevic Fariz/);
assert.match(indexHtml, /tel:\+38268400509/);
...
assert.doesNotMatch(adminHtml, /Admin panel je otvoren, ali neki podaci trenutno nisu dostupni/);
assert.doesNotMatch(adminHtml, /<option[^>]*>Enterprise<\/option>/);
assert.match(adminHtml, /billing_cycle/);
assert.match(adminHtml, /Godišnje -/);
assert.match(adminHtml, /Email za obavijesti vlasniku/);
...
console.log('functional tests: admin session includes credentials, server-backed annual discount preview and stable package selectors');
